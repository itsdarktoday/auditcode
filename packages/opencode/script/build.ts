...
// ~/.auditcode/skills/bundled on first run / version bump. Embedding (vs the
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/auditcode`,
      execArgv: [`--user-agent=auditcode/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
      ...(embeddedFileMap ? { "opencode-web-ui.gen.ts": embeddedFileMap } : {}),
      "auditcode-skills.gen.ts": embeddedSkillsMap,
    },
    entrypoints: [
      "./src/index.ts",
      parserWorker,
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/auditcode`
...
if (Script.release) {
  ...
  // seeded to ~/.auditcode/skills/bundled on first run — no skills.tar.gz asset.
