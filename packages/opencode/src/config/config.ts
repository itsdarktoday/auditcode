...
import { Flag } from "@auditcode/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { InstallationLocal, InstallationVersion } from "@auditcode/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@auditcode/core/v1/config/console-state"
import { FSUtil } from "@auditcode/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@auditcode/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@auditcode/core/v1/config/config"
import { RemoteAuthError } from "@auditcode/core/v1/config/error"
import { ConfigPermissionV1 } from "@auditcode/core/v1/config/permission"
import { ConfigPluginV1 } from "@auditcode/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@auditcode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["auditcode.jsonc", "auditcode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "auditcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "auditcode.jsonc"), env))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/auditcode`
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("auditcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".auditcode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["auditcode.json", "auditcode.jsonc", "opencode.json", "opencode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.auditcode/plugin(s)` are already local files, so ConfigPlugin.load
