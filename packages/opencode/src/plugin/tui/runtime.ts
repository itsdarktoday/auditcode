...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@auditcode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@auditcode/core/effect/app-node-builder"
import { errorData, errorMessage } from "@auditcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@auditcode/tui/context/theme"
import { Global } from "@auditcode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@auditcode/core/util/flock"
import { Flag } from "@auditcode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@auditcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@auditcode/core/v1/config/plugin"
import { createCommandShim } from "@auditcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@auditcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".auditcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".auditcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".auditcode", "tui.json"),
  }
}
