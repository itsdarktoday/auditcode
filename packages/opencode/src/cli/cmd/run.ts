import type { PermissionV1 } from "@auditcode/core/v1/permission"
import { FSUtil } from "@auditcode/core/fs-util"
// CLI entry point for `auditcode run` and `auditcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`auditcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`auditcode --mini --attach`): connects to a running
//      auditcode server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@auditcode/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run auditcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running auditcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'auditcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://auditcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
