...
import { UI } from "../ui"
import { ModelsDev } from "@auditcode/core/models-dev"
...
const AGENT_USERNAME = "auditcode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/auditcode.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/ptc summarize` to see the agent in action",
            "",
            "   Learn more about the GitHub agent - https://github.com/itsdarktoday/auditcode",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/auditcode-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.github.com/repos/${app.owner}/${app.repo}/installation`)
            .then((res) => res.json())
...
jobs:
  auditcode:
    if: |
      contains(github.event.comment.body, ' /ptc') ||
      startsWith(github.event.comment.body, '/ptc') ||
      contains(github.event.comment.body, ' /auditcode') ||
      startsWith(github.event.comment.body, '/auditcode')
    ...
    steps:
      ...
      - name: Run auditcode
        uses: itsdarktoday/auditcode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://github.com/itsdarktoday/auditcode" : "https://github.com/itsdarktoday/auditcode"
    ...
    try {
      ...
      console.log("auditcode session", session.id)
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/auditcode,/ptc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to auditcode...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("auditcode-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `auditcode/${type}-${hex}-${timestamp}`
      }
      return `auditcode/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/auditcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[auditcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the auditcode infrastructure after your response",
