...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall auditcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AuditCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g auditcode-ai",
      pnpm: "pnpm uninstall -g auditcode-ai",
      bun: "bun remove -g auditcode-ai",
      yarn: "yarn global remove auditcode-ai",
      brew: "brew uninstall auditcode",
      choco: "choco uninstall auditcode",
      scoop: "scoop uninstall auditcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "auditcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".auditcode") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AuditCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# auditcode") || content.includes("# opencode") || content.includes(".auditcode/bin") || content.includes(".opencode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# auditcode" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".auditcode/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".auditcode/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".auditcode") || trimmed.includes(".opencode")))
    ) {
      continue
    }
