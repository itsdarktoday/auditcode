...
import { MAX_STEPS_PROMPT } from "@auditcode/core/session/runner/max-steps"
...
import { SessionTable } from "@auditcode/core/session/sql"
...
const modeDirectives: Record<string, string> = {
  auto: "Mode: AUTO — Drive the smart contract audit autonomously across all 6 phases. Run scoping, static analysis, dispatch specialist auditor subagents, verify with PoC tests, and generate the final audit report inline.",
  free: "Mode: FREE — Operator directed. Execute what the operator asks (e.g. check specific contract function, analyze specific reentrancy or math vector), report results, suggest next steps only when asked.",
  guided: "Mode: GUIDED — Step-by-step audit walkthrough. Propose each audit step (e.g. scanning contracts, dispatching math auditor, writing Foundry PoC) and wait for operator approval before proceeding.",
  deep_scan: "Mode: DEEP SCAN — Exhaustive verification with automated PoC generation for every candidate lead.",
}
...
function phaseTransitionHint(state: EngagementSchema.State): string | undefined {
  ...
  switch (state.current_phase) {
    case "scope_recon":
    case "recon":
      if (s.contracts_count > 0 || s.hosts_discovered > 0) return "Scope recon completed with in-scope contracts mapped. Consider advancing to static analysis phase (/phase next)."
      break
    case "static_analysis":
    case "enumeration":
      if (s.vulnerabilities_total > 0) return "Static analysis completed and initial findings ingested. Consider advancing to threat modeling phase (/phase next)."
      break
    case "threat_modeling":
      if (s.invariants_total > 0 || Object.keys(state.actors ?? {}).length > 0) return "Threat modeling and invariants formalized. Consider advancing to deep audit phase (/phase next)."
      break
    case "deep_audit":
    case "vuln_assess": {
      const allVulns = [...Object.values(state.vulns ?? {}), ...Object.values(state.hosts).flatMap(h => h.vulns)]
      const confirmed = allVulns.some((v) => v.status === "confirmed" || v.status === "lead")
      if (confirmed) return "Deep audit complete with candidate findings. Consider advancing to PoC verification phase (/phase next)."
      break
    }
    case "poc_verification":
    case "exploitation":
    case "post_exploit":
      if (s.pocs_total > 0 || s.vulnerabilities_total > 0) return "PoC verification completed. Ready to generate the final assessment report (/phase next or /report)."
      break
  }
  return undefined
}
...
export class Service extends Context.Service<Service, Interface>()("@auditcode/SessionPrompt") {}
