...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@auditcode/sdk/v2"
...
export const AuthMethodID = "auditcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@auditcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `auditcode auth login` in the terminal",
      name: "Login with auditcode",
      id: AuthMethodID,
    }
...
async function loadDirectorySnapshot(sdk: OpencodeClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    return Directory.build({
      ...
      providers,
      modes,
      defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "pentest",
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "AuditCode prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "AuditCode service failure", service })
}
