...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@auditcode/plugin"
...
import { SlitherParseTool } from "./slither-parse"
import { AderynParseTool } from "./aderyn-parse"
import { FoundryTestTool } from "./foundry-test"
import { ContractInspectTool } from "./contract-inspect"
...
export class Service extends Context.Service<Service, Interface>()("@auditcode/ToolRegistry") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const slitherparse = yield* SlitherParseTool
    const aderynparse = yield* AderynParseTool
    const foundrytest = yield* FoundryTestTool
    const contractinspect = yield* ContractInspectTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          slitherparse: Tool.init(slitherparse),
          aderynparse: Tool.init(aderynparse),
          foundrytest: Tool.init(foundrytest),
          contractinspect: Tool.init(contractinspect),
