import type { TuiPluginApi } from "@auditcode/plugin/tui"
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to share the current session",
  ...
  "Create {highlight}auditcode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/auditcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.auditcode/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.auditcode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.auditcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.auditcode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent AuditCode from reading sensitive files",
  "Use {highlight}auditcode run{/highlight} for non-interactive scripting",
  "Use {highlight}auditcode --continue{/highlight} to resume the last session",
  "Use {highlight}auditcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}auditcode serve{/highlight} for headless API access to AuditCode",
  "Use {highlight}auditcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}auditcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}auditcode auth list{/highlight} to see all configured providers",
  "Run {highlight}auditcode agent create{/highlight} for guided agent creation",
  "Use {highlight}/status{/highlight} to check engagement status, contracts, and vulns",
  "Use {highlight}/scope{/highlight} to view or update the engagement scope",
  "Use {highlight}/phase next{/highlight} to advance to the next audit phase",
  "Use {highlight}/report{/highlight} to generate an audit report from findings",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.auditcode/themes/{/highlight} directory",
  ...
  "Run {highlight}auditcode debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/contracts{/highlight} to list in-scope smart contracts, SLOC, and functions",
  "Use {highlight}/vulns{/highlight} to see discovered vulnerabilities by severity",
  "Use {highlight}/invariants{/highlight} to view evaluated protocol invariants and test status",
  "Use {highlight}/pocs{/highlight} to view verified exploit proof-of-concept tests",
  "Use {highlight}/mode auto{/highlight} for autonomous auditing, {highlight}/mode free{/highlight} for user-directed",
  "Use {highlight}/pause always{/highlight} to review each finding before the agent continues",
