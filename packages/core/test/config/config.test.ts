...
import { ConfigProvider } from "@auditcode/core/config/provider"
import { AppNodeBuilder } from "@auditcode/core/effect/app-node-builder"
import { LayerNode } from "@auditcode/core/effect/layer-node"
import { ConfigMigrateV1 } from "@auditcode/core/v1/config/migrate"
import { ConfigV1 } from "@auditcode/core/v1/config/config"
...
import { Policy } from "@auditcode/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads auditcode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "auditcode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "auditcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "auditcode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "auditcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "auditcode.json")
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "auditcode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "auditcode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "auditcode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .auditcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".auditcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".auditcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "auditcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "auditcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "auditcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "auditcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "auditcode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".auditcode", "auditcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".auditcode", "auditcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".auditcode")),
              AbsolutePath.make(path.join(directory, ".auditcode")),
            ])
