...
import { define } from "@auditcode/plugin/v2/effect"
...
import { LocationServiceMap } from "@auditcode/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "auditcode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "auditcode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
